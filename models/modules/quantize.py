...
QParams = namedtuple('QParams', ['scale', 'qmin', 'qmax', 'two_power_of_radix'])
...
def quant_round_constrain(t1, t2, trange, tzp):
    qp = QParams(scale = 1.0, qmin = -127, qmax = 127, two_power_of_radix = 1)
    t1q = quantize_with_grad(t1, num_bits=8, qparams=qp, dequantize=False)
    t2q = quantize_with_grad(t2, num_bits=8, qparams=qp, dequantize=False)
...
def calculate_qparams(x, num_bits, flatten_dims=_DEFAULT_FLATTEN, reduce_dim=0,  reduce_type='mean', keepdim=False, true_zero=False,per_ch_input=False,quant_mode = 'maxmin'):
    ...
    with torch.no_grad():
        ...
        return     QParams(scale = None, qmin = -127, qmax = 127, two_power_of_radix = 1) #no longer used.
...
class UniformQuantize(InplaceFunction):
    ...
    def forward(ctx, input, num_bits=None, qparams=None, flatten_dims=_DEFAULT_FLATTEN,
        ...
        # radix =output.new_tensor(qparams.radix) 
        ...
        output.mul_(qparams.scale)
        output.mul_(qparams.two_power_of_radix)
...
def quantize_with_grad(input, num_bits=None, qparams=None, flatten_dims=_DEFAULT_FLATTEN, reduce_dim=0,clamp=True, dequantize=True, signed=False, stochastic=False, inplace=False,quant_zp=QZP):
    ...
    # num_bits = torch.reshape(output.new_tensor(qparams.num_bits), inshape)
    # scale = qparams.output_scale
    # output_scale = qparams.output_scale
    # radix =torch.reshape(output.new_tensor(qparams.radix), inshape)
    # if qparams.output_scale is None and qparams.input_scale is None: 
    #     scale = 1.0
    # elif qparams.output_scale is None: #is datapath input, 
    #     scale = output.new_tensor(qparams.input_scale)
...
class QConv2dSamePadding(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        self.quantize_input = QuantMeasure(
            self.num_bits, shape_measure=(out_channels, num_measure, 1, 1), flatten_dims=(1, -1), measure=measure,per_ch_input=per_ch_input)
    ...
    def forward(self, input):
        ...
        if self.bias is not None:
            qbias = self.bias if self.measure else quantize(self.bias, num_bits=self.num_bits_weight,flatten_dims=(0, -1)) #TODO: add qparams
        else:
            qbias = None
...
class QConv2d_lapq(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        self.quantize_weight = QuantMeasure(
            self.num_bits, shape_measure=(out_channels, in_channels, 1, 1), flatten_dims=(1, -1), measure=measure, reduce_dim=None, weight_flag=True)
        self.biprecision = biprecision
    ...
    def forward(self, input):
        ...
        if self.bias is not None:
            qparams_bias = QParams(scale = self.quantize_weight.running_scale, qmin = self.quantize_weight.bias_qmin, qmax = self.quantize_weight.bias_qmax, two_power_of_radix = self.quantize_weight.bias_two_power_of_radix)
...
class QConv2dVQ(nn.Conv2d):
    ...
    def forward(self, input):
        ...
        qvx = self.quantize_input(vx.view(C,B,H,W).transpose(1,0).contiguous(), qparams=qparams_input).transpose(0,1).contiguous().view(C,-1) if self.quantize else input
        ...
        if not self.measure and os.environ.get('DEBUG')=='True':
            ...
            assert  qweight[0].unique().numel()<=2**self.num_bits_weight
        if self.bias is not None:
            qparams_bias = QParams(scale = self.quantize_weight.running_scale, qmin = self.quantize_weight.bias_qmin, qmax = self.quantize_weight.bias_qmax, two_power_of_radix = self.quantize_weight.quantize_weight.bias_two_power_of_radix)
...
class QLinear_o(nn.Linear):
    ...
    def __init__(self, in_features, out_features, bias=True, num_bits=8, num_bits_weight=8, num_bits_grad=None, biprecision=False,measure=False):
        ...
        self.num_bits_grad = num_bits_grad
        self.biprecision = biprecision
        self.quantize_input = QuantMeasure(self.num_bits,measure=measure, shape_measure=(out_features, in_features))
        self.measure = measure
...
class QLinear_lapq(nn.Linear):
    ...
    def __init__(self, in_features, out_features, bias=True, num_bits=8, num_bits_weight=8, num_bits_grad=None, biprecision=False,measure=False):
        ...
        self.biprecision = biprecision
        self.quantize_input = QuantMeasure(self.num_bits,measure=measure,shape_measure=(out_features, in_features))
        self.quantize_weight = QuantMeasure(self.num_bits,shape_measure=(out_features, in_features), measure=measure,reduce_dim=None, weight_flag=True)
        self.measure = measure
...
class QLinear(nn.Linear):
    ...
    def __init__(self, in_features, out_features, bias=True, num_bits=8, num_bits_weight=8, num_bits_grad=None, perC=True, biprecision=False,measure=False, cal_qparams=False):
        ...
        if measure:
            self.quantize_input = QuantMeasure(self.num_bits,measure=measure, shape_measure=(out_features if perC else 1, in_features), cal_qparams=cal_qparams)
            self.quantize_weight = QuantMeasure(self.num_bits,shape_measure=(out_features if perC else 1, in_features), flatten_dims=(1,-1) if perC else (0,-1), measure=measure,reduce_dim=None if perC else 0, weight_flag=True)
        else:
            self.quantize_input = QuantThUpdate(self.num_bits,measure=measure, shape_measure=(in_features, in_features))
            self.quantize_weight = QuantThUpdate(self.num_bits,shape_measure=(out_features if perC else 1, in_features), flatten_dims=(1,-1) if perC else (0,-1), measure=measure,reduce_dim=None if perC else 0, weight_flag=True)
        self.measure = measure
        self.cal_params = cal_qparams
        self.quantize = QUANTIZE
...
class QLinearVQ(nn.Linear):
    ...
    def __init__(self, in_features, out_features, bias=True, num_bits=8, num_bits_weight=8, num_bits_grad=None, perC=True, biprecision=False,measure=False, cal_qparams=False):
        ...
        if measure:
            self.quantize_input = QuantMeasure(self.num_bits,measure=measure, shape_measure=(out_features if perC else 1, 1), cal_qparams=cal_qparams)
            ...
            self.quantize_input = QuantThUpdate(self.num_bits,measure=measure,shape_measure=(out_features if perC else 1, 1))
    ...
    def forward(self, input):
        ...
        qweight = self.quantize_weight(self.weight, qparams=qparams_weight) if self.quantize and not self.cal_params else self.weight
