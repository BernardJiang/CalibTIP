...
def adaquant(layer, cached_inps, cached_outs, test_inp, test_out, lr1=1e-4, lr2=1e-2, iters=100, progress=True, batch_size=64, relu=False, writer=None):
    ...
    lr_w = 1e-3 # mse_before.cpu().detach().numpy() # 1e-5 # 0.0025 # 1e-6 #lr_factor * layer.weight.std().item()  # 1e-5
    lr_b = 1e-4 # mse_before.cpu().numpy() # 1e-5 # 0.0025 # 1e-6#lr_factor * layer.bias.std().item()  # 1e-3
    weight_decay = 0.01
...
def optimize_layer(layer, in_out, optimize_weights=False, batch_size=100, model_name=None, writer=None):
    ...
    if optimize_weights:
        ...
        mse_before, mse_after = adaquant(layer, cached_inps, cached_outs, test_inp, test_out, iters=50, batch_size=batch_size, lr1=1e-5, lr2=1e-4, relu=relu_flag, writer=writer) 
        mse_before_opt = mse_before
